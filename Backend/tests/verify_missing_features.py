...
def verify_sensor_data_endpoint():
    url = "http://localhost:7000/sensor-data"
    print(f"\nFetching {url}...")
    try:
        response = requests.get(url)
        if response.status_code == 200:
...
def verify_fault_injection():
    url = "http://localhost:7000/machine/control"
    payload = {
...
if __name__ == "__main__":
    ...
    try:
        requests.post("http://localhost:7000/machine/control", json={"command": "start"})
    except:
        pass
